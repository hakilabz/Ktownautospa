// Server-side Rate Card & Pricing Engine for Ktown Auto Spa (2026 Official Rate Card)
// Single source of truth for pricing, package inclusions, and overlap validation
...
export const RATE_CARD = {
  washAndDetail: {
    'hand-car-wash': {
      ...
      qualifiesForWithDetail: false,
    },
    'interior-refresh': {
      title: 'Interior Refresh',
      desc: 'Doors, jambs, panels and dashboard wiped down, full blow-out and vacuum, rubber mats washed or carpet mats dry-cleaned. No steam, no scrubbing — the quick tidy-up',
      duration: 'Approx. 1 hr',
      prices: { 'c-sedan': 70, 'c-cross': 85, 'c-suv': 95, 'c-van': 105 },
      qualifiesForWithDetail: true,
    },
    'medium-package': {
    ...
    'interior-complete': {
      ...
      desc: 'Everything we do inside — 305°F steam extraction through the whole car, seats and carpets shampooed, vinyl and trim cleaned and dressed. The only thing it leaves out is the exterior wash — add a hand wash for $30',
  ...
  polishAndProtect: {
    'gloss-enhancement': {
      ...
      desc: 'Our wax package. Clay bar, then a one-step all-in-one polish that cleans, polishes and protects the paint and takes out light swirls',
      duration: 'Approx. 3 hrs',
  ...
  ceramicCoating: {
    ...
    'maint-plan': {
      title: 'Maintenance Plan (Billed Monthly)',
  ...
  addons: {
    ...
    'spray-wax': { title: 'Spray Wax (with any wash)', price: 25 },
    ...
    'pet-hair': { title: 'Pet Hair Removal', price: 50 },
    'extra-dirty': { title: 'Extra Dirty Vehicle Surcharge', price: 50 },
    'extra-coating-layer': { title: 'Additional Ceramic Coating Layer', price: 150 },
    'interior-complete-wash': { title: 'Add-on Hand Car Wash (with Interior Complete)', price: 30 },
  },
};
...
// Check if any item in the cart qualifies for "With a Detail" pricing on Interior/Glass/Wheel/Trim
export function cartQualifiesForWithDetail(cartItems = []) {
  for (const item of cartItems) {
    const sid = item.serviceId;
    if (
      RATE_CARD.washAndDetail[sid]?.qualifiesForWithDetail ||
      RATE_CARD.polishAndProtect[sid]?.qualifiesForWithDetail ||
      RATE_CARD.ceramicCoating[sid]?.qualifiesForWithDetail
    ) {
      return true;
    }
  }
...
export function calculateOrderTotals(cartItems = [], discountAmount = 0, paymentMethod = 'pay_at_dropoff') {
  ...
  const hasDetailOrCeramic = cartQualifiesForWithDetail(cartItems);
  ...
  for (const item of cartItems) {
    ...
    if (serviceDef) {
      ...
      // Apply "With a detail" price if cart has a detail/ceramic package OR if explicitly flagged
      const useWithDetail = hasDetailOrCeramic || item.isStandalone === false;
      itemPrice = useWithDetail ? intDef.withDetailPrice : intDef.standalonePrice;
      serviceDef = intDef;
    } else if (RATE_CARD.addons[item.serviceId]) {
      const addDef = RATE_CARD.addons[item.serviceId];
      itemPrice = addDef.price;
      serviceDef = addDef;
    ...
    // Add selected add-ons attached to this item
    let addonsTotal = 0;
