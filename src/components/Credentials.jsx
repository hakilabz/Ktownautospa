...
export default function Credentials() {
  const certs = [
    {
      icon: <ShieldCheck style={{ width: '1.4rem', height: '1.4rem', color: 'var(--navy-deep)' }} />,
    ...
    {
      icon: <Award style={{ width: '1.4rem', height: '1.4rem', color: 'var(--navy-deep)' }} />,
    ...
    {
      icon: <Star style={{ width: '1.4rem', height: '1.4rem', color: 'var(--navy-deep)' }} />,
    ...
    {
      icon: <FileText style={{ width: '1.4rem', height: '1.4rem', color: 'var(--navy-deep)' }} />,
  ...
  const brandTiles = [
    ...
    },
    {
      ...
      bg: 'linear-gradient(180deg, #16386C, #0A1E42)',
      color: '#FFFFFF',
      dark: true,
    },
    {
      brand: 'SYSTEM X',
      label: 'Accredited Installation Center',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem' }}>
            {certs.map((c, idx) => (
              <div key={idx} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                ...
                <div>
                  <strong style={{ display: 'block', fontSize: '1.04rem', color: 'var(--navy-deep)', marginBottom: '0.2rem' }}>
                    {c.title}
                  </strong>
                  <span style={{ display: 'block', color: 'var(--slate)', fontSize: '0.92rem', lineHeight: 1.5 }}>
                    {c.desc}
                  </span>
                </div>
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
            {brandTiles.map((tile, idx) => (
              <div 
                ...
                style={{
                  borderRadius: '12px',
                  border: tile.dark ? '2px solid var(--gold)' : '2px solid var(--cream-3)',
              ...
              >
                ...
                <b style={{ fontFamily: 'var(--mono)', fontSize: '0.76rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: tile.dark ? 'var(--gold-lt)' : 'var(--slate)', display: 'block', marginBottom: '0.2rem' }}>
                  {tile.label}
                </b>
                <span style={{ fontSize: '0.78rem', color: tile.dark ? '#A9C4E2' : 'var(--slate)', opacity: 0.85 }}>
                  {tile.tag}
                </span>
              </div>
            ))}
